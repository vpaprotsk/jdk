...
public final class MontgomeryIntegerPolynomialP256 extends IntegerPolynomial implements IntegerMontgomeryFieldModuloP, IntegerResidueMontgomeryFieldModuloP { 
    ...
    private void debugRow(String prefix, long[] c, int start, int length) {
        if (true) {
            return;
        }
        final java.util.HexFormat hex = java.util.HexFormat.of();
    ...
    protected void mult(long[] a, long[] b, long[] r) {
        ...
        long k, nm;
        debugRow("a", a, 0, NUM_LIMBS);
        debugRow("b", b, 0, NUM_LIMBS);

        for(int row = 0; row < NUM_LIMBS; row++) {
            for(int col = 0; col < NUM_LIMBS; col++) {
                // Row - multiply by aa1 and reduce out c1
                d[col] = a[row] * b[col]; 
                dd[col] = Math.unsignedMultiplyHigh(a[row], b[col]) << shift1 | (d[col] >>> shift2); 
                d[col] &= LIMB_MASK; 
            }
            d[0] += c[row+0]; 
            nm = d[0] & LIMB_MASK;
        ...
        c[0] = c[NUM_LIMBS] - modulus[0];
        for(int col = 1; col < NUM_LIMBS; col++) {
            c[col] = c[col+NUM_LIMBS] - modulus[col] + (c[col-1] >> BITS_PER_LIMB); c[col-1] &= LIMB_MASK;
        }

        long mask = c[NUM_LIMBS-1] >> BITS_PER_LIMB; // Signed shift!
        //                                                //c9 &= LIMB_MASK;
        // debugRow("reduced\n", c0, c1, c2, c3, c4);
        debugRow("reduced", c, 0, NUM_LIMBS);
        assert(mask==0 || mask==-1);

        for(int col = 0; col < NUM_LIMBS; col++) {
            r[col] = ((c[col+NUM_LIMBS] & mask) | (c[col] & ~mask)) & LIMB_MASK;
        }
    }
    ...
    protected void multUnrolled(long[] a, long[] b, long[] r) {
        ...
        r[1] =  ((c6 & mask) | (c1 & ~mask)) & LIMB_MASK;
        r[2] =  ((c7 & mask) | (c2 & ~mask)) & LIMB_MASK;
        r[3] =  ((c8 & mask) | (c3 & ~mask)) & LIMB_MASK;
        r[4] =  ((c9 & mask) | (c4 & ~mask)) & LIMB_MASK;
    }
    ...
    private static final long[] one     = new long[]{ 0x0000000000000001L, 0x000ff00000000000L, 0x000fffffffffffffL, 0x000fffefffffffffL, 0x000000000000ffffL};
