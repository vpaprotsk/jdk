...
// ATTRIBUTE_ALIGNED(64) uint64_t BROADCAST0[] = {
//   0x0000000000000007, 0x0000000000000007,
//   0x0000000000000007, 0x0000000000000007,
//   0x0000000000000007, 0x0000000000000007,
//   0x0000000000000007, 0x0000000000000007,
// };
// static address broadcast_0() {
//   return (address)BROADCAST0;
// }
...
void StubGenerator::montgomeryMultiply(const Register aLimbs, const Register bLimbs, const Register rLimbs) {
  ...
  */    
  ...
  // __ mov64(t0, 0x0f);
  // __ kmovql(bottomLimbs, t0);
  __ evmovdquq(shift1L, allLimbs, ExternalAddress(shift_1L()), false, Assembler::AVX_512bit, rscratch); //endiannes?
  __ evmovdquq(shift1R, allLimbs, ExternalAddress(shift_1R()), false, Assembler::AVX_512bit, rscratch); //endiannes?
  __ evmovdquq(broadcast5, allLimbs, ExternalAddress(broadcast_5()), false, Assembler::AVX_512bit, rscratch); //endiannes?
  __ evmovdquq(mask52, allLimbs, ExternalAddress(p256_mask52()), false, Assembler::AVX_512bit, rscratch); //endiannes?
  ...
  __ evmovdquq(Address(rLimbs, 0), allLimbs, Acc1, true, Assembler::AVX_512bit); //SIGILL?? but _mm512_mask_storeu_epi64 exists!
}
